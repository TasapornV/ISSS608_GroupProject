...
cons_yr <- readRDS(file = "RDS/chosendata.rds")
 cons_year <- cons_yr %>%
        group_by(type, year) %>%
...
The second line uses the **`find_k`** function from the **`Dendextend`** package to calculate the optimal number of clusters based on the within-cluster sum of squares (WSS) method. This function takes the hierarchical clustering object as input and returns the optimal number of clusters.
...
```{r}
clus        <- readRDS(file = "RDS/clus.rds")
clus_group1 <- clus[,-c(2)] %>%
    group_by(Description) %>%
    summarise_each(list(sum))
  ...
  # Making it into a matrix
  clus_matrix1 <- data.matrix(clus_group1)
  saveRDS(clus_matrix1, file = "RDS/clus_matrix1.rds")
  saveRDS(clus_group1, file = "RDS/clus_group1.rds")
