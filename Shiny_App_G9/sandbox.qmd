...
T3.5 <- readRDS(file = "RDS/T3-5.rds") # Average Monthly Household Electricity Consumption by Planning A
```
...
#   filter(month != "Annual") %>% 
#   group_by(Date) %>% 
#   summarise(sum = sum(as.numeric(kwh_per_acc)))
# arima3 <- arima3 %>% 
#   group_by(Date) %>% 
...
full_arima_mth_agg2 = full_arima2 %>%
  # index_by(Date = ~ Date(.)) %>% 
  summarise(
    avg_demand = mean(sum, na.rm = TRUE)
  )


head(full_arima_mth_agg)
head(full_arima_mth_agg2)
```
...
arima_ts2 <- ts(data=arima2$sum, start = c(2005,1), end = c(2022, 6), frequency=12)
plot.ts(arima_ts2, plot.type=('single'))     
```
...
# Fit ARIMA model
fit <- arima(arima_ts, order = c(1,0,1))

# Compute forecast
fc <- forecast(fit)

# Plot forecast and state estimates
plot(fc, plot.conf = FALSE)
lines(fit$states[, 1], col = 'red')

```

```{r}
# fit <- ets(window(arima_ts))
# fit2 <- ets(window(arima_ts2))
...
```{r}
# fit <- ets(y = selectedData1(),model = switch(input$models,
#                                                    ANN = "ANN",
#                                                    ANA = "ANA",
#                                                    AMM = "AMM",
#                                                    AAN = "AAN",
...
arima_ds2 = diff(arima_ts2, differences = 2)
ggplotly(plot(arima_ds2))
```
...
arima_arima2 <- auto.arima(arima_ts2)
arima_arima2
plot(forecast(arima_arima2))
...
library(dendextend)
library(stats)
a <- normalize(clus_group1[, -1])

clustering <- dist(a,  method = "euclidean")
...
Duplicate: Interactive Sparklines for a time period (Bandline/Mean)
...
Duplicate: Interactive Sparklines for a time period (Mean)
...
# Final (Intro) Library and Data Prep
...
Dwelling Type Data Preparation
...
# Final (Intro): Toggle between Dwelling and Town
...
# Final (Overview): Line Chart

Specifying the dataset and time period
...
# Final (Overview): Slopegraph between any two years
...
Backup: Interactive Sparklines for a time period -Bandline
...
Backup: Interactive Sparklines for a time period (Mean)
...
# Final (Overview): Report+Sparkline based on same timeframe as the chart
