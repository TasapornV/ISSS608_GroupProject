...
# Final (Dwelling): Cycle Plot for a dwelling type
...
# Final (Dwelling): Slopegraph between any two years
...
# Final (Dwelling): Interactive Sparklines for a time period -Bandline
...
# Final (Dwelling): Interactive Sparklines for a time period -Mean
...
# Final (Dwelling): Report based on same timeframe as the chart
...
town$date <- as.Date(town$date, format="%d-%m,-%Y") 
...
# Final (Town): Boxplot for electricity consumption by town for a selected year
...
# Final (Town): For click and show - boxplot by dwelling type

```{r}
town_boxplot <- town %>% 
  filter(year %in% c(inputyr)) %>%
  filter(Description %in% c('Ang Mo Kio'))

#PLotting the chart
dwelling_boxplot %>%
  mutate(dwelling_type = fct_reorder(dwelling_type, kwh_per_acc)) %>%
  ggplot( aes(x=dwelling_type, y=kwh_per_acc, fill=dwelling_type)) +
    geom_boxplot() +
    scale_fill_viridis(discrete = TRUE, alpha=0.6) +
#    theme_ipsum() +
    theme(
    ...
    ggtitle(paste0("Electricity Consumption by each dwelling in the Town, Year ",inputyr)) +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  xlab("") +
  ylab("Consumption (kWh)")
```

# Final (Town): Interactive Sparklines for a time period -Bandline

```{r}
#data formatting
t_sparks <- town %>%
  filter(year %in% c(startyear:endyear)) %>%
  mutate(`Town` = Description) %>%
  group_by(`Town`) %>%
  summarize(`Monthly Consumption` = list(kwh_per_acc))
...
town %>%
  filter(year %in% c(startyear:endyear)) %>%
  group_by(Description) %>%
  summarise("Min" = min(kwh_per_acc, na.rm = T),
            "Max" = max(kwh_per_acc, na.rm = T),
            "Average" = mean(kwh_per_acc, na.rm = T)
            ) %>%
  gt() %>%
  fmt_number(columns = 4,
    decimals = 2)
