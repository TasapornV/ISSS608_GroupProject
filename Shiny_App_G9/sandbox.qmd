...
a <-ETS(peak_system_demand_mw)
a <- model(ETS(peak_system_demand_mw))
a <- model(.arima$peak_system_demand_mw)
arima$peak_system_demand_mw
arima %>%
  model(ETS(peak_system_demand_mw)) %>%
...
# Clustering
library(dtwclust)
hc <- tsclust(CharTraj, type = "hierarchical",  
              distance = "dtw_basic",  
